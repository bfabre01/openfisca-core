...
class Test(TestCase):
    ...
    def setUp(self):
        ...
        self.extended_subject = create_app(tax_benefit_system).test_client()

    def test_return_code(self):
        parameters_response = self.extended_subject.get('/parameters')
        assert_equal(parameters_response.status_code, OK)

    def test_return_code_existing_parameter(self):
        extension_parameter_response = self.extended_subject.get('/parameter/local_town.child_allowance.amount')
        assert_equal(extension_parameter_response.status_code, OK)

    def test_return_code_existing_variable(self):
        extension_variable_response = self.extended_subject.get('/variable/local_town_child_allowance')
